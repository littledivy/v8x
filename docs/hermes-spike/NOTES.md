...
- [x] *** E14 STARTUP TRUTH (user challenged the E13 "slow" number - correctly). Decomposed the ~30-35ms boot_trivial cold-start: ~6.5ms process/dyld (framework ~1.5ms), ~26ms try_new = SOURCE-PARSING the deno_core+ext bootstrap JS (~14ms core + ~12ms web/net/fetch), ~0.2ms eval. NOT the engine (0.25ms), NOT mostly dyld. The parse tax is the killer. HBC lever on the REAL boot JS (23 files, 638KB, E1 lowering + hermesc -O): source ~21-29ms -> HBC ~1.7-3.7ms = ~8-12x, ~19-25ms parse recovered. Fair cold-start (31 procs same harness): Hermes-HBC-boot 7.4ms / V8 deno 14.0 / QuickJS deno 21.4 / Hermes-minimal-source 21.3 / Hermes-src-boot 27.4 / Hermes-full-source 34.5. CRITICAL CAVEAT: the 7.4ms/HBC number STUBS execution (bodies run vs a __bootstrap Proxy) - it measures PARSE removal, not a real full boot. HBC removes parse NOT execution; V8's snapshot removes BOTH (pre-built heap); Hermes has no heap-snapshot equivalent. So a real HBC-boot-with-execution is NOT yet measured (inference: ~10-15ms, competitive w/ V8, better than QuickJS - unproven). HONEST: engine fast + parse tax real + HBC removes parse; full-boot-beats-V8 UNPROVEN due to snapshot/execution gap. Wins = size + embedding + parse-free short scripts. static-link saves ~1.5ms. 54/54 backend. See experiments/E14. ***
- [x] *** E13 REAL RELEASE BINARY + honest full-startup/size numbers. Fixed the release-build panic: fat-LTO cross-crate-inlined the extern-C embedder-data shims into rusty_v8 get_annex_mut, dissolving the FFI barrier -> stale len=0 reused after Set -> assert(0>1). Fix = #[inline(never)] on the 3 Context embedder-data shims (0a19b08). 53/53 backend. Release binary BOOTS + runs a real Deno program. MEASURED (stripped, median 25 procs): Hermes minimal runtime (deno_core+web/net/fetch) = 7.6MB binary (+3-6MB framework = 10.5-13.6MB total), cold start ~30-35ms. vs V8 deno 82.7MB/16.5ms, QuickJS deno 70.9MB/24.1ms (same harness). HONEST: (1) 7.6MB is a MINIMAL runtime, NOT the full deno CLI (no TS/swc, no snapshot, no full ext) - a full deno-on-Hermes projects to ~QuickJS-scale ~68MB, runtime-dominated; the tiny-engine win is for EMBEDDING not the CLI. (2) STARTUP IS NOT FAST YET: 30-35ms is SLOWER than V8/QuickJS full binaries because the ext JS is parsed from SOURCE each launch; the 23x HBC parse-free win is NOT wired into the binary. Lever to actually demonstrate fast startup = ship the ext bootstrap as HBC. See experiments/E13. ***
- [x] *** E12 REAL HTTP on Hermes (re-verified): a real JS HTTP server SERVES real HTTP correctly - curl/warm request -> 200 "hello world", ApacheBench 30k reqs 0 failed (round-trip re-verified: __served=2001, PASS). BUT no FAIR rps: Hermes is DEBUG + JS accept loop (~4.6k conc1 / 9.2k conc50, flat vs concurrency = loop-bound) vs QuickJS/V8 RELEASE + native Deno.serve (45k trivial close, 71k/189k keep-alive). Compute handler 213 Hermes / 394 QuickJS / 3774 V8. E11 "write-buffer marshaling gap" was a MISDIAGNOSIS: real cause = a genuine Hermes engine defect (loop-scoped const/let bound after await, captured by a detached async closure, reads undefined on resume -> connRid undefined -> #[smi] rid decode threw); fixed by passing state as a fn argument (arg-frame slots survive await). 51/51 backend. Fair release rps BLOCKED by a release-build JsRuntime::try_new embedder-data panic (follow-up). See experiments/E12. ***
- [x] *** E11 BENCHMARKS (honest, re-verified by orchestrator): is Hermes worth adding as a Deno engine? WINS = SIZE + STARTUP, not compute. (1) NO JIT in the vendored framework: nm finds 0 JitEmitter symbols; enabling withEnableJIT+ForceJIT+JITThreshold=0 moves compute 0.5% (noise). Interpreter-only. (2) SIZE: Hermes engine 6.34MB arm64 vs QuickJS-deno 68MB / V8-deno 79MB - ~order of magnitude smaller. (3) STARTUP: parse-free HBC 23x on bootstrap-shaped JS (215ms->9.3ms), engine-level (no Hermes deno binary yet; not a full-Deno cold start). (4) COMPUTE (re-run): Hermes fib32x5 1051ms vs QuickJS 557ms = 1.9x SLOWER; loopsum 1.2x slower; json 2x FASTER; 3-18x behind V8. (5) HTTP: real JS server blocked on op2 write-buffer marshaling gap -> NO rps claimed (honest); proxy handler-cost Hermes 225/QuickJS 422/V8 4060 rps. VERDICT: worth it for small-footprint fast-cold-start edge/serverless/mobile IO-bound; NOT a compute engine in this build; the "small like QuickJS but faster via JIT" pitch is FALSE here. 49/49 backend. See experiments/E11. ***
- [x] *** E10 CAPSTONE: a REAL DENO PROGRAM runs end-to-end on Hermes. A user ES module (file:///main.js) loaded via deno_core's REAL module loader (load_main_es_module + async mod_evaluate + run_event_loop, NOT execute_script) runs fetch("http://...")+r.text() + async generator + for-await + new URL().pathname + console.log + TOP-LEVEL AWAIT, all in one module evaluation on one event loop, output asserted "BODY: hello-from-hermes SUM: 6 URL: /y". Backend gap fixed (f5c99ef): modeled module body was a plain closure so TLA was a SyntaxError -> Module::Evaluate wrap in async function, pin+return the real promise. 45/45 backend from clean build. The north star ("a static Hermes backend that can run Deno") is reached for a real multi-feature program. See experiments/E10. ***
- [x] *** E9: fetch() WORKS over real HTTP on Hermes. `await fetch("http://127.0.0.1/")` -> status 200, body "hello-from-hermes" through real 26_fetch.js -> op_fetch (hyper) -> op_fetch_send -> r.text() draining the response ReadableStream over a real socket, via run_event_loop. Backend gap fixed (c060d6e): Latin-1 String read ABI (ContainsOnlyOneByte/IsOneByte/WriteOneByte_v2 were null stubs -> serde_v8 ByteString decode threw ExpectedLatin1 on even "GET"). Backend 44/44 from clean build, NO cppgc dup (confirms E8's was a stale artifact). No regression (E4-E8 all still pass). TLS/https deferred to E10. See experiments/E9. ***
- [x] *** E8: THE ORIGINAL CEILING IS VINDICATED. `for await (const conn of listener)` accepts a REAL connection and a byte round-trips client->server->client over a real loopback OS socket, through real ext/net, driven by run_event_loop on Hermes (TCP ROUND-TRIP PASS; FOR-AWAIT PASS accepted=true count=1). E7's "uv_compat reactor" diagnosis was WRONG: (A) a probe bug (fresh per-test tokio runtime orphaned the op poll_task) + (B) a REAL backend gap - v8__Uint32__Value/v8__Int32__Value were null stubs with the wrong C-ABI sig, so .value() returned 0, zeroing every non-zero async-op promiseId (E6's op_delayed only worked because its id was 0). Fixed via ToUint32/ToInt32 (9a9d916). Backend suite 43/43 green (the agent's "cppgc dup blocks suite" was a stale-artifact race; independently re-ran clean). Timer path + all 7 ext/web round-trips still pass. Next (E9): fetch. See experiments/E8. ***
- [~] *** E7: for-await over a socket BLOCKED - but NOT by Hermes. Real deno_net (ext/net) builds + boots on Hermes; op_net_listen returns a real listener; accept/connect dispatch real promises; the `for await (const conn of listener)` construct RUNS (reaches "listening", dispatches op_net_accept, suspends). Socket op promises never settle through run_event_loop. Root cause (reproducing probe + architect): this scratch checkout's custom libuv reactor (libs/core/uv_compat) doesn't service kqueue-readiness or spawn_blocking wakes while the loop parks - only timer wakes get through (why E6 worked). Raw tokio sockets + spawn_blocking work on the SAME runtime OUTSIDE run_event_loop. So this is a deno-checkout event-loop integration bug, not the engine, not the op->promise bridge. Fixed a real Hermes lowering bug en route (fe060b8: lower ONLY async gens, not ES2022 #private-fields). 42/42 backend. E8: fix the uv_compat reactor (chain park to tokio I/O + blocking unpark). See experiments/E7. ***
- [x] *** E6: REAL DEFERRED-OP ASYNC works on Hermes. A genuinely-deferred async op (awaits tokio sleep, pending on first poll) settles its JS promise through the real deno_core run_event_loop: await op_delayed(41) === 42 (~7ms real sleep, deferred confirmed). Root cause was v8__Function__Call failing to decode a Global<Function> pin-handle -> silently blocked deno_core's __eventLoopTick for EVERY deferred op; fixed (67a75ec, slot_of_handle). Also: console.log -> "hi { a: 1 } [ 2, 3 ]" via real op_console_inspect_args (+ reflection ABI chain); URL parse -> "/p/q|1|#h". 41/41 backend. This is the op->promise->loop foundation for timers/fetch/net. Console CLASS still blocked on cppgc. Next (E7): ext/net + fetch (for-await over a real socket). See experiments/E6. ***
- [x] *** E5: ext/web substantially FUNCTIONAL incl streams + structuredClone. Root-caused the ReadableStream "Pending": a Promise::State observation lag (not a drain gap); fixed so state-read drains once (63ae403). reader.read() on an enqueued ReadableStream -> "value=42,done=false" through real 06_streams.js + deno_core event loop. v8::ValueSerializer/Deserializer (25 symbols, 3f4b93b) -> structuredClone deep-equals + refuses unsupported types cleanly. 39/39 backend. Honest boundary: settles ALREADY-resolved promises; a promise resolved LATER by a deferred op (timer/net) on run_event_loop is UNTESTED - that's the real for-await-over-socket bridge (E6). See experiments/E5. ***
- [x] *** E4: ext/web is now partially FUNCTIONAL on Hermes (not just hydrated). v8::Private (67491c1) -> thrown ext/web errors FORMAT cleanly instead of panicking; TypedArray/ArrayBufferView ABI (dbacb3f) + v8::Symbol (c61a865) -> TextEncoder/TextDecoder round-trip "héllo (bytes=6)" through real ext/web JS. queueMicrotask shim added. 37/37 backend. Deferred walls: structuredClone (needs v8::ValueSerializer), ReadableStream read (promise stays Pending - event-loop/microtask drive). See experiments/E4. ***
- [x] *** E3: deno_webidl + deno_web BOOT on Hermes and ALL 24 ext/web lazy JS files evaluate without throwing (incl 8000-line 06_streams.js + async-gen 09_file.js). Six backend gaps fixed (7e3752e): CompileFunction, Global pin refcount, non-JSError capture, Exception::CreateMessage/String::Empty/Object::GetPrototype, + ext-script `return`-wrapper lowering fix. Functional: atob/btoa, Event "test:true", ReadableStream.getReader. 35/35 backend tests. Next wall (E4): v8::Private (ForApi/Get/SetPrivate null -> error formatter panics) + TextEncoder ArrayBuffer ABI (op_encoding_encode_into "expected ArrayBuffer or ArrayBufferView"). See experiments/E3. ***
- [x] *** E2: verified vs REAL Deno. deno_core boots + runs 1+1 WITH the real E1 lowering active (E1 had regressed it - the D8 "1+1" was a stale pre-E1 binary; fixed in b746874 by giving lowered async gens the %AsyncGeneratorFunction.prototype% chain). Async iteration inside booted deno_core = "1,2,3,4"; ext/web-shaped streaming = "1,2,3,4,5"; primordials AsyncGeneratorPrototype{Next,Return,Throw} real. Full deno_runtime blocked by a NON-Hermes deno_napi E0282; deno_web builds clean. All gates independently re-verified. See experiments/E2. ***
