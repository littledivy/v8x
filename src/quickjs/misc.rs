...
// ICU common-data loader (vendored rusty_v8 `icu::set_common_data_77`). V8
// needs its bundled icudtl.dat; QuickJS brings its own ICU/Intl, so loading
// v8's blob is a no-op. Report success (error_code = 0) so `set_common_data_77`
// returns Ok and test setup proceeds instead of segfaulting on a null symbol.
#[unsafe(no_mangle)]
