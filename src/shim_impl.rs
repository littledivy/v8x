...
/// `Deno.versions.v8` for this backend: the REAL JavaScriptCore version instead
/// of a fabricated V8 number. JSC has no C version API, so read the system
/// JavaScriptCore.framework bundle version via CoreFoundation (already linked).
/// Built once and leaked. Falls back to the engine name if the lookup fails.
#[unsafe(no_mangle)]
...
fn jsc_version_string() -> std::ffi::CString {
    ...
    let ver = unsafe {
        ...
        if !bundle.is_null() {
            let key =
                CFStringCreateWithCString(std::ptr::null(), c"CFBundleVersion".as_ptr(), UTF8);
