...
// FfiResult layout MUST match src/simdutf.rs exactly:
//   #[repr(C)] struct FfiResult { error: i32, count: usize }
#[repr(C)]
...
// simdutf error codes (mirrors ErrorCode in src/simdutf.rs)
const SUCCESS: i32 = 0;
...
pub extern "C" fn simdutf__validate_utf8_with_errors(
  ...
  match core::str::from_utf8(s) {
    Ok(_) => ok(len),
    Err(e) => {
      // Distinguish "truncated tail" (TooShort) from a genuine bad byte.
...
// Validate UTF-16 given an iterator of native-endian code units.
...
#[unsafe(no_mangle)]
pub extern "C" fn simdutf__validate_utf32(
  buf: *const u32,
  len: usize,
) -> bool {
...
// ---------------------------------------------------------------------------
// UTF-16 decoding helper: iterate scalar values from native-endian units.
// Returns the scalar values for valid input; for transcoding "valid_*"
// variants we assume validity.
// ---------------------------------------------------------------------------
...
// Encode a scalar value as UTF-8 into out; returns bytes written.
#[inline]
...
// ---------------------------------------------------------------------------
// Conversion: UTF-8 <-> UTF-16LE / BE
// ---------------------------------------------------------------------------

// Convert UTF-8 to UTF-16 (native scalars -> code units with `from_native`).
fn utf8_to_utf16(
  input: &[u8],
  output: *mut u16,
...
// UTF-8 -> Latin-1: every scalar must be <= 0xFF.
fn utf8_to_latin1(input: &[u8], output: *mut u8) -> Result<usize, (i32, usize)> {
  ...
  let mut n = 0usize;
  // Iterate with byte offsets so error positions are exact.
...
pub extern "C" fn simdutf__convert_latin1_to_utf8(
  ...
  unsafe {
    for &b in s {
      // Latin-1 byte -> scalar value == byte
...
pub extern "C" fn simdutf__convert_utf32_to_utf8(
  ...
  // Validate first; return 0 on any invalid scalar.
...
fn utf8_len_from_utf16(units: &[u16], to_native: impl Fn(u16) -> u16) -> usize {
  ...
  while i < units.len() {
    ...
    if (0xD800..=0xDBFF).contains(&u) && i + 1 < units.len() {
      let lo = to_native(units[i + 1]) as u32;
      if (0xDC00..=0xDFFF).contains(&lo) {
        total += 4; // surrogate pair -> 4-byte UTF-8
        i += 2;
        continue;
      }
    }
    // lone unit (BMP or unpaired surrogate counted as 3 bytes, matching simdutf)
    total += if u < 0x80 {
      1
    } else if u < 0x800 {
      2
...
pub extern "C" fn simdutf__utf16_length_from_utf8(
  ...
  let s = unsafe { slice_u8(input, length) };
  // Number of UTF-16 code units = count of leading bytes that aren't
  // continuation bytes, plus one extra per 4-byte sequence (surrogate pair).
  ...
  for &b in s {
    ...
    if b >= 0xF0 {
      count += 1; // 4-byte sequence needs a surrogate pair
    }
  }
  count
}
...
pub extern "C" fn simdutf__utf8_length_from_latin1(
  ...
  let s = unsafe { slice_u8(input, length) };
  // Each byte >= 0x80 needs 2 UTF-8 bytes, others 1.
...
pub extern "C" fn simdutf__latin1_length_from_utf8(
  ...
  let s = unsafe { slice_u8(input, length) };
  // Number of code points = number of non-continuation bytes.
...
pub extern "C" fn simdutf__utf32_length_from_utf8(
  ...
  let s = unsafe { slice_u8(input, length) };
  // One UTF-32 unit per code point = per non-continuation byte.
  s.iter().filter(|&&b| (b & 0xC0) != 0x80).count()
}
...
pub extern "C" fn simdutf__utf32_length_from_utf16le(
  ...
  let s = unsafe { slice_u16(input, length) };
  // One scalar per non-low-surrogate-leading unit; pair counts as one.
  let mut count = 0usize;
  let mut i = 0;
  while i < s.len() {
...
pub extern "C" fn simdutf__count_utf8(
  ...
  let s = unsafe { slice_u8(input, length) };
  // Count code points = non-continuation bytes.
  s.iter().filter(|&&b| (b & 0xC0) != 0x80).count()
}
...
pub extern "C" fn simdutf__count_utf16le(
  ...
  let s = unsafe { slice_u16(input, length) };
  // Count code points: low surrogates do not start a new code point.
  s.iter()
...
pub extern "C" fn simdutf__detect_encodings(
  ...
  // encoding bitmask: UTF8=1, UTF16_LE=2, UTF16_BE=4, UTF32_LE=8, UTF32_BE=16
  let s = unsafe { slice_u8(input, length) };
  let mut mask = 0i32;

  // BOM-based fast paths (mirrors simdutf behaviour).
  ...
  if length % 2 == 0 {
    let u16s = unsafe {
      core::slice::from_raw_parts(input as *const u16, length / 2)
    };
  ...
  if length % 4 == 0 {
    let u32s = unsafe {
      core::slice::from_raw_parts(input as *const u32, length / 4)
    };
...
// Base64
// ---------------------------------------------------------------------------
// Options (u64): Default=0 (std+pad), Url=1 (urlsafe no pad),
//   DefaultNoPadding=2 (std no pad), UrlWithPadding=3 (urlsafe + pad).
// LastChunkHandling: Loose=0, Strict=1, StopBeforePartial=2, OnlyFullChunks=3.
...
fn b64_decode_value(c: u8, url: bool) -> Option<u8> {
  match c {
    ...
    b'-' if url => Some(62),
    b'_' if url => Some(63),
    // Accept both alphabets leniently for the non-url variant on +/-_ too.
    b'-' => Some(62),
    b'_' => Some(63),
    _ => None,
  }
...
pub extern "C" fn simdutf__maximal_binary_length_from_base64(
  ...
  let s = unsafe { slice_u8(input, length) };
  // Count significant (non-whitespace, non-padding) characters.
  let sig = s
    .iter()
    .filter(|&&c| !c.is_ascii_whitespace() && c != b'=')
    .count();
  // Each 4 base64 chars -> 3 bytes; ceil to be safe (maximal).
  (sig / 4) * 3 + ((sig % 4) * 3 + 3) / 4
}
...
pub extern "C" fn simdutf__base64_to_binary(
  ...
  // Collect 6-bit values, ignoring ASCII whitespace; stop at first '='.
  ...
  // Strict / OnlyFullChunks handling of a partial trailing group.
  if rem == 1 {
    // A single leftover sextet can never form a byte.
    return err(BASE64_INPUT_REMAINDER, sextets.len());
  }
  if last_chunk_options == 1 && rem != 0 {
    // Strict: incomplete final chunk is an error.
    return err(BASE64_INPUT_REMAINDER, sextets.len());
  }
  ...
  unsafe {
    ...
    // Handle trailing partial group unless told to stop/only-full.
    let handle_partial =
      rem != 0 && last_chunk_options != 2 && last_chunk_options != 3;
    if handle_partial {
      let i = full_groups * 4;
      if rem == 2 {
        // 12 bits -> 1 byte; low 4 bits must be zero in strict-ish modes.
...
pub extern "C" fn simdutf__base64_length_from_binary(
  ...
  if wants_padding(options) {
    // 4 chars per 3 bytes, padded to a multiple of 4.
    ((length + 2) / 3) * 4
  } else {
    // No padding: ceil(length*8 / 6) = ceil(length*4/3).
    (length * 4 + 2) / 3
  }
}
