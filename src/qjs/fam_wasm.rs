...
/// WAMR's vector layout (see WASM_DECLARE_VEC in wasm_c_api.h): 5 fields.
#[repr(C)]
...
pub struct wasm_val_t {
    pub kind: u8,
    pub _pad: [u8; 7],
    pub of: u64, // union; reinterpret per kind
}
...
unsafe extern "C" {
    ...
    pub fn wasm_module_new(store: *mut wasm_store_t, binary: *const wasm_vec_t)
        -> *mut wasm_module_t;
    pub fn wasm_module_validate(store: *mut wasm_store_t, binary: *const wasm_vec_t) -> bool;
    ...
    pub fn wasm_importtype_module(it: *const wasm_importtype_t) -> *const wasm_vec_t;
    pub fn wasm_importtype_name(it: *const wasm_importtype_t) -> *const wasm_vec_t;
    pub fn wasm_importtype_type(it: *const wasm_importtype_t) -> *const wasm_externtype_t;
    ...
    pub fn wasm_exporttype_name(et: *const wasm_exporttype_t) -> *const wasm_vec_t;

    pub fn wasm_functype_params(ft: *const wasm_functype_t) -> *const wasm_vec_t;
    pub fn wasm_functype_results(ft: *const wasm_functype_t) -> *const wasm_vec_t;
    ...
    pub fn wasm_trap_new(store: *mut wasm_store_t, message: *const wasm_vec_t)
        -> *mut wasm_trap_t;

    // Globals (e.g. wasm-bindgen / es-module-lexer export `__heap_base`,
    // `__stack_pointer` etc. that JS reads via `.value`).
    ...
    pub fn wasm_globaltype_content(gt: *const wasm_globaltype_t) -> *const wasm_valtype_t;
    pub fn wasm_globaltype_mutability(gt: *const wasm_globaltype_t) -> u8;

    // Tables (for WASM-ESM externref tables used by wasm-bindgen).
    pub fn wasm_extern_as_table(e: *mut wasm_extern_t) -> *mut wasm_table_t;
    pub fn wasm_table_size(t: *const wasm_table_t) -> u32;
    pub fn wasm_table_grow(t: *mut wasm_table_t, delta: u32, init: *mut wasm_ref_t) -> bool;
    pub fn wasm_table_get(t: *const wasm_table_t, index: u32) -> *mut wasm_ref_t;
    pub fn wasm_table_set(t: *mut wasm_table_t, index: u32, r: *mut wasm_ref_t) -> bool;

    // Foreign refs carry a host pointer — we box a JSValue and store it, so a JS
    // value round-trips through a wasm externref table slot.
    ...
    // `wasm_foreign_as_ref` allocates a FRESH wasm_ref_t (by index) that does NOT
    // inherit the foreign's host_info, so host_info must be set on the ref itself —
    // that ref pointer is what WAMR's externref map round-trips (obj2ref stores it,
    // ref2obj returns the same pointer), and what `wasm_ref_get_host_info` reads.
...
/// Finalizer for a boxed JSValue stored as a foreign-ref's host info.
unsafe extern "C" fn externref_box_free(p: *mut c_void) {
    if !p.is_null() {
        unsafe { drop(Box::from_raw(p as *mut JSValue)) };
        // NOTE: the JSValue's refcount is intentionally leaked here — the QuickJS
        // runtime is being torn down when foreign refs finalize, so freeing
        // against a dead context would be unsafe. Externref tables are tiny.
    }
}

/// Box a (dup'd) JSValue for storage as a foreign-ref host pointer.
...
/// Recover a JSValue from a foreign-ref host pointer (borrowed; not freed).
...
/// Encode a JS value as a wasm externref host pointer (a `wasm_ref_t*`).
///
/// WAMR round-trips this pointer opaquely (host hashmap key), so the same
/// encoding must be used everywhere externrefs cross the boundary —
/// function args/results, import args/results, and table slots — otherwise a
/// value set one way and read another would mis-decode. We box the (dup'd)
/// JSValue as a foreign-ref's host info, exactly like `table_set`.
unsafe fn js_to_externref(ctx: *mut JSContext, v: JSValue) -> u64 {
    let store = with_state(|st| st.store);
    // A foreign-kind ref (so `wasm_table_set` accepts it). `wasm_foreign_as_ref`
    // mints a FRESH ref object — set host_info on THAT ref (not the foreign), since
    // the ref pointer is what WAMR's externref map round-trips and what
    // `externref_to_js` reads back.
...
/// Decode a wasm externref host pointer (`wasm_ref_t*`) back to a JS value.
/// Returns a fresh (dup'd) reference. NULL refs decode to `undefined`.
...
thread_local! {
    ...
    /// Original JS exception stashed by `import_trampoline` when a JS import threw
    /// (it returns a wasm trap to unwind the guest). `call_export` re-throws this
    /// so the real error propagates instead of a generic "trap" message.
    static PENDING_IMPORT_EXC: RefCell<Option<JSValue>> = const { RefCell::new(None) };
    /// Nesting depth of `call_export` — detects wasm re-entrancy (a host import
    /// during a wasm call synchronously calling back into wasm).
...
/// Read a wasm binary from a JS value (ArrayBuffer / TypedArray / Buffer).
unsafe fn read_wasm_bytes(ctx: *mut JSContext, v: JSValue) -> Option<Vec<u8>> {
    // TypedArray (Uint8Array / Node Buffer) → underlying ArrayBuffer + offset.
    let mut off = 0usize;
    let mut len = 0usize;
    let mut bpe = 0usize;
    let ab = unsafe { JS_GetTypedArrayBuffer(ctx, v, &mut off, &mut len, &mut bpe) };
...
/// Convert a wasm value to a JS value.
...
/// Convert a JS value to a wasm value of the given kind.
unsafe fn js_to_wasm_val(ctx: *mut JSContext, kind: u8, v: JSValue) -> wasm_val_t {
...
fn vec_name_to_string(vec: *const wasm_vec_t) -> std::string::String {
    ...
    // WAMR's name vectors include the trailing NUL terminator in `size`; trim it
    // (and any others) so CString construction / property names are clean.
    let mut len = v.size;
...
// ===================================================================
// Import-function bridge (JS function called from wasm)
// ===================================================================

struct ImportEnv {
    ctx: *mut JSContext,
    func: JSValue, // duped; leaked for process lifetime
    result_kinds: Vec<u8>,
...
unsafe extern "C" fn import_trampoline(
    ...
    if std::env::var("V82_WASM_TRACE").is_ok() {
        ...
        eprintln!("[wasm] import_trampoline argc={} name={:?} depth={d}", js_args.len(), env.name);
    }
    let ret = unsafe {
        JS_Call(
            ctx,
    ...
    // If the JS import threw, surface it as a wasm trap rather than writing a
    // bogus result (which would let the guest run on with a garbage externref
    // and typically `unreachable`-trap deep inside wasm-bindgen). Returning a
    // trap unwinds the guest cleanly so the call boundary throws in JS. We stash
    // the ORIGINAL exception so `call_export` can re-throw it (preserving the real
    // error) instead of a generic "trap during function call".
    if ret.tag == JS_TAG_EXCEPTION {
        ...
        if std::env::var("V82_WASM_TRACE").is_ok() {
            ...
            if !cs.is_null() {
                let b = unsafe { std::slice::from_raw_parts(cs as *const u8, l) };
                eprintln!("[wasm] IMPORT THREW: {}", std::string::String::from_utf8_lossy(b));
                unsafe { JS_FreeCString(ctx, cs) };
            }
        }
    ...
    // Write results (single or none; multi-value returns an array — rare here).
...
// ===================================================================
// JS-callable: exported wasm function wrapper
// ===================================================================
...
unsafe extern "C" fn call_export(
    ...
    // data[0] holds the ExportFuncEnv pointer encoded as a BigInt.
    let mut envp = 0i64;
    ...
    if depth > 1 && std::env::var("V82_WASM_TRACE").is_ok() {
        eprintln!("[wasm] *** RE-ENTRANT call_export depth={depth} into {:?}", env.name);
    }
    struct DepthGuard;
    ...
    let mut results: Vec<wasm_val_t> = vec![
        wasm_val_t { kind: 0, _pad: [0; 7], of: 0 };
    ...
    // Drop any exception a (nested) JS import threw during this call — the guest
    // either trapped (handled below) or caught/recovered and returned normally.
    ...
    if results.is_empty() {
        ...
        if std::env::var("V82_WASM_TRACE").is_ok() {
            let dump: Vec<(u8, u64)> = results.iter().map(|r| (r.kind, r.of)).collect();
...
/// Build a JS callable wrapping an exported wasm function.
unsafe fn make_export_func(ctx: *mut JSContext, f: *mut wasm_func_t, name: &str) -> JSValue {
...
unsafe fn table_env_of(ctx: *mut JSContext, data: *mut JSValue) -> *const TableEnv {
    let mut envp = 0i64;
    unsafe { JS_ToBigInt64(ctx, &mut envp, *data) };
    envp as *const TableEnv
}

/// `table.grow(delta)` — grow the wasm table by `delta`, return the previous size
/// (wasm-bindgen uses the return as the new region's base offset).
...
/// `table.set(index, value)` — store `value` (a JS externref) in the table slot.
unsafe extern "C" fn table_set(
    ...
    if !foreign.is_null() {
        let r = unsafe { wasm_foreign_as_ref(foreign) };
        if !r.is_null() {
            // Set host_info on the ref (see `js_to_externref`): the foreign's own
            // host_info isn't visible through the as_ref copy that gets stored.
...
/// `table.get(index)` — recover the JS externref stored in a table slot.
unsafe extern "C" fn table_get(
    ...
    if std::env::var("V82_WASM_TRACE").is_ok() {
        ...
        eprintln!("[wasm] table_get idx={idx} r_null={} hi_null={}", r.is_null(), hi.is_null());
    }
    if r.is_null() {
        return jsv_undefined();
    }
...
/// A WebAssembly.Table export object exposing `grow`/`get`/`set`/`length`.
unsafe fn make_table_obj(ctx: *mut JSContext, table: *mut wasm_table_t) -> JSValue {
    ...
    unsafe {
        ...
        JS_SetPropertyStr(ctx, obj, c"length".as_ptr(), JS_NewInt32(ctx, len as i32));
    }
    obj
}

// ===================================================================
// Module / Instance construction
// ===================================================================

unsafe fn compile_module(ctx: *mut JSContext, bytes: &[u8]) -> Result<usize, JSValue> {
    with_state(|st| {
...
/// Compile wasm `bytes` into a Module JS object (the same `__wasm_module_id`
/// wrapper `new WebAssembly.Module` produces, so `new WebAssembly.Instance` /
/// `obj_module_id` accept it). Used by `v8__WasmModuleObject__Compile` for the
/// WASM-ESM `import source` integration. Returns a JS exception value on failure.
pub(crate) unsafe fn compile_module_object(ctx: *mut JSContext, bytes: &[u8]) -> JSValue {
...
/// Create a Module JS object wrapping module id.
unsafe fn make_module_obj(ctx: *mut JSContext, id: usize) -> JSValue {
    let obj = unsafe { JS_NewObject(ctx) };
    unsafe {
        JS_SetPropertyStr(ctx, obj, c"__wasm_module_id".as_ptr(), JS_NewInt32(ctx, id as i32))
    };
    obj
}
...
unsafe fn obj_module_id(ctx: *mut JSContext, obj: JSValue) -> Option<usize> {
    ...
    // Only a real Module wrapper has this numeric property; anything else
    // (undefined for e.g. a Uint8Array) must NOT be treated as id 0.
...
/// Instantiate module `mid` with `import_object`, return the exports object.
unsafe fn instantiate(
    ...
    // Build the imports vector in the module's declared order.
    ...
    for i in 0..n {
        ...
        // Look up import_object[modname][name].
        ...
        if kind == WASM_EXTERN_FUNC && unsafe { JS_IsFunction(ctx, val) } != 0 {
            ...
            let f = unsafe {
                wasm_func_new_with_env(store, ft, import_trampoline, env as *mut c_void, None)
            };
            externs.push(unsafe { wasm_func_as_extern(f) });
        } else {
            // Unsupported / missing import (memory/global/table from JS). Push
            // null — instantiation will trap if the module truly needs it.
    ...
    if inst.is_null() {
        ...
        return Err(unsafe { throw(ctx, "WebAssembly.Instance: instantiation failed") });
    }
    ...
    for i in 0..count {
        let name = vec_name_to_string(unsafe { wasm_exporttype_name(*et_data.add(i)) });
...
unsafe extern "C" {
    /// Install the post-grow hook in WAMR (vendor patch in wasm_memory.c). Fires
    /// after every successful linear-memory grow (opcode OR host API).
    fn v82jsc_set_mem_grow_cb(cb: Option<unsafe extern "C" fn()>);
}

/// Memory object with a live `buffer` ArrayBuffer view over wasm linear memory.
/// We cache the handed-out ArrayBuffer and DETACH it on grow (`on_mem_grow`), so
/// JS holders (notably wasm-bindgen, which only refetches `memory.buffer` once its
/// cached view's `byteLength` reaches 0) see the detach and refresh — otherwise a
/// view created before a grow keeps the old, short length → "RangeError: out of
/// bound". (Our linear memory no longer MOVES on grow — see wasm_memory.c's 4GB
/// reserve — so the only staleness left is the view length, fixed by detach.)
struct MemoryEnv {
...
thread_local! {
    /// Every live `MemoryEnv` (leaked for process lifetime), so the global grow
    /// hook can detach all their cached buffers.
...
/// WAMR post-grow hook: detach every memory's cached ArrayBuffer so stale JS views
/// (byteLength now 0) force a refetch of `memory.buffer`.
...
/// `memory.buffer` getter — returns an ArrayBuffer over the current wasm memory.
/// Stable between grows (V8 semantics): the cached buffer is reused until a grow
/// detaches it, then the next access mints a fresh one over the new size.
unsafe extern "C" fn memory_buffer_get(
    ...
    // Non-owning view: wasm owns the memory; no free func.
    let buf = unsafe { JS_NewArrayBuffer(ctx, dptr, size, None, ptr::null_mut(), false) };
    // Hold our own ref so we can detach it later; hand a dup to JS.
    env.cached.set(unsafe { JS_DupValue(ctx, buf) });
    buf
}

/// `WebAssembly.Memory.prototype.grow(delta)` — grow linear memory by `delta`
/// 64KiB pages, returning the PREVIOUS size in pages (V8 semantics). Required by
/// e.g. es-module-lexer, which calls `memory.grow(n)` before writing source into
/// the heap; without the method `memory.grow` is `undefined` → "not a function".
unsafe extern "C" fn memory_grow(
    ...
    if !ok {
        return unsafe { throw(ctx, "WebAssembly.Memory.grow: failed to grow memory") };
    }
...
/// `WebAssembly.Global.prototype.value` getter — read the global's current value.
unsafe extern "C" fn global_value_get(
    ...
    let mut v = wasm_val_t { kind: 0, _pad: [0; 7], of: 0 };
    unsafe { wasm_global_get(env.global, &mut v) };
    unsafe { wasm_val_to_js(ctx, &v) }
}

/// `WebAssembly.Global.prototype.value` setter — write a mutable global.
...
/// A `WebAssembly.Global` export object exposing a live `value` accessor.
unsafe fn make_global_obj(ctx: *mut JSContext, global: *mut wasm_global_t) -> JSValue {
    let obj = unsafe { JS_NewObject(ctx) };
    let genv = Box::into_raw(Box::new(GlobalEnv { global }));
    let mut gd = [unsafe { JS_NewBigInt64(ctx, genv as i64) }];
    let getter = unsafe { JS_NewCFunctionData(ctx, global_value_get, 0, 0, 1, gd.as_mut_ptr()) };
    let mut sd = [unsafe { JS_NewBigInt64(ctx, genv as i64) }];
    let setter = unsafe { JS_NewCFunctionData(ctx, global_value_set, 1, 0, 1, sd.as_mut_ptr()) };
...
unsafe fn make_memory_obj(ctx: *mut JSContext, mem: *mut wasm_memory_t) -> JSValue {
    // Install the WAMR post-grow hook once (detaches stale `memory.buffer`s).
    ...
    let obj = unsafe { JS_NewObject(ctx) };
    // ONE shared env for both the `buffer` getter and `grow` (so the cached buffer
    // and detach see the same state). Leaked for process lifetime; registered so
    // the grow hook can reach it.
    ...
    let getter = unsafe { JS_NewCFunctionData(ctx, memory_buffer_get, 0, 0, 1, data_arr.as_mut_ptr()) };
    ...
    let grow_fn = unsafe { JS_NewCFunctionData(ctx, memory_grow, 1, 0, 1, gdata.as_mut_ptr()) };
    unsafe { JS_SetPropertyStr(ctx, obj, c"grow".as_ptr(), grow_fn) };
    obj
}

// ===================================================================
// JS entry points (WebAssembly.*)
// ===================================================================
...
unsafe extern "C" fn wa_instance_ctor(
    ...
    let Some(mid) = (unsafe { obj_module_id(ctx, module) }) else {
        return unsafe { throw(ctx, "WebAssembly.Instance: argument is not a Module") };
    };
...
unsafe extern "C" fn wa_compile(
    ...
    // Synchronously compile, return a resolved/rejected promise.
...
unsafe extern "C" fn wa_instantiate(
    ...
    // If first is a Module, resolve with an Instance. Else treat as bytes and
    ...
    match unsafe { instantiate(ctx, mid, imports) } {
        Ok(inst) => {
            ...
            unsafe {
                JS_SetPropertyStr(ctx, res, c"module".as_ptr(), make_module_obj(ctx, mid))
            };
...
/// Wrap a value (or pending exception) into a settled Promise via a cached
/// `(v, isErr) => isErr ? Promise.reject(v) : Promise.resolve(v)` helper.
unsafe fn settle_promise(ctx: *mut JSContext, value: JSValue) -> JSValue {
    ...
    let p = unsafe { JS_Call(ctx, helper, jsv_undefined(), 2, args.as_mut_ptr()) };
    unsafe { JS_FreeValue(ctx, v) };
    p
}
...
/// Install `globalThis.WebAssembly`. Called once per context at isolate setup.
pub(crate) fn install_webassembly(ctx: *mut JSContext) {
    unsafe {
        ...
        // Module/Instance/Memory are constructors (`new WebAssembly.Module(...)`).
        let set_ctor =
            |obj: JSValue, name: &std::ffi::CStr, f: JSCFunction, argc: c_int| {
                let func =
